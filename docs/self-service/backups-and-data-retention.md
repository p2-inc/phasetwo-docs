...
Your cluster's data — realms, users, clients, sessions, and configuration — lives in a database we operate and back up. Backups run on a regular schedule, are encrypted, and are stored separately from the production data they protect. Restores are tested periodically, and our business continuity and disaster recovery plan is tested at least annually.
