// Sidebar for the /api section, which carries two distinct APIs.
//
// The split is by *what the API talks to*, not by product area:
//
//   Management API  -> the Phase Two control plane. Creates and configures
//                      clusters themselves. Generated from
//                      openapi-management.yaml, a build artifact of
//                      phasetwo-keycloak.
//   Extensions API  -> the customer's own Keycloak realm. Phase Two's
//                      additions to the Keycloak Admin REST API. Generated
//                      from openapi.yaml.
//
// The Extensions API entries stay at the top of api/ rather than moving under
// an api/extensions/ prefix, because those 84 endpoint URLs are published and
// indexed -- relocating them would break every inbound link for the sake of
// symmetry. The Management API is new, so it gets the nested path.
//
// The Management API's items are generated: `pnpm regen-api-docs` rewrites
// api/management/sidebar.js from the spec. The Extensions API's are still
// hand-maintained here, which is why regen deletes api/sidebar.ts and leaves
// this file alone.
const managementItems = require("./api/management/sidebar.js");

module.exports = {
  api: [
    // Hand-written section root. Serves at /api/ via `slug: /` in its
    // frontmatter, which is what stops /api/ from 404ing while the endpoint
    // pages sit under it. Not generated, so it survives a regeneration.
    {
      type: "doc",
      id: "api-index",
    },
