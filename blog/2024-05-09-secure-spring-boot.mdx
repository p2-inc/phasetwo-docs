...
[SpringBoot](https://spring.io/projects/spring-boot) is a open-source tool which uses Java based frameworks for building web applications.
...
Starting with SpringBoot 2.x the Keycloak client adapters were deprecated. In SpringBoot 3.x we will use native functionalities of the `spring-boot-oauth2-resource-server` to be able to configure the application security context.
