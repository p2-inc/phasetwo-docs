...
2. Configure Spring Boot resource server
   ...
   In `/src/main/java/com/springbootkeycloak` create a new folder, `config`, and create a file `SecurityConfig.java` within the FileSystemDirectoryReader. In that File, add the `HttpSecurity` settings configuration class:
...
3. Add JWT token convert configuration
   ...
   In the same `config` folder, create another File, `JwtClaimsConverter.java`. Add a converter for extracting the security context attributes from the `access_token` received from Keycloak.
...
4. Create the secured API resources:
   ...
   In `/src/main/java/com/springbootkeycloak` create a new folder, `web`, and create a file `TestController.java`.
   ...
   Start the application running with `./gradelw bootRun`.
